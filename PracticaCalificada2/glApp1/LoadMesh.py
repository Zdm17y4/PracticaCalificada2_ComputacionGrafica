    ...
    def load_drawing(self, filename):
        ...
        with open(filename) as fp:
            ...
            while line:
                ...
                if line[:2] == "vt":
                    # Some OBJ exporters include a third coordinate (w). Accept 2 or 3 values and keep (u, v).
                    vals = [float(value) for value in line[3:].split() if value.strip() != '']
                    if len(vals) >= 2:
                        uvs.append((vals[0], vals[1]))
                    else:
                        # fallback to 0,0 if malformed
                        uvs.append((0.0, 0.0))
                if line[:2] == "f ":
